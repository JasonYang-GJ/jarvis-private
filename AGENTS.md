...
Build a Windows learning assistant that observes one user-selected window and gives step-by-step Chinese guidance. The MVP is observation-only: it must not click, type, move the pointer, or invoke destructive actions.
...
- Capture only the selected window in the MVP, not the entire desktop.
