...
状态：**Final Freeze C0 候选。** S4-R1、S4-R2、S4-R3 与 S4-R4 均已 `INTEGRATED_PASS`；V0.6.0 版本/安装包元数据与预冻结文档正在形成 C0。真实 `v0.6.0-stage4`、独立标签源码构建和仅文档 C1 证据尚未产生，因此当前不代表正式冻结或对外发布；S4-R5 与 Stage 5 均未开始。
...
- Stage 4 Final Freeze（C0 候选）：版本统一到 V0.6.0，计划标签 `v0.6.0-stage4`；标签、安装包哈希/大小/文件数只能在后续独立标签源码构建后由 C1 记录。同 AppId 生命周期、数字签名和语音模型许可/分发仍未放行；
