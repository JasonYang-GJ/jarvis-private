# 元枢产品事实（V0.4.0 冻结基线 / V2 阶段 3 R2 候选）

> 当前产品事实的唯一入口。更新时间：2026-08-30。V0.4.0 阶段 2 已冻结；S3-R1 已集成，当前源码正在验证阶段 3 的 S3-R2 本地预览候选，它不是新的正式发布版本。阶段 2 安装包身份仍以 `docs/baselines/V0.4.0_STAGE2.md` 为准。
...
- **阶段 3 R1 / R2**：已有只由用户显式管理、仅保存在本机的加密长期记忆账本；R2 候选新增用户点击触发的确定性本地相关记忆预览。当前不会自动提取、后台检索或发送给模型；RAG、向量数据库、跨 Session 自动个性化和用户画像仍未实现。
...
- S3-R2 只有显式、确定性的本机预览；没有自动记忆提取、模型上下文注入、后台/语义检索、RAG、向量数据库、用户画像或跨 Session 自动个性化。
