...
- SQLite/Desktop IPC 当前候选合同为 v10。v9 → v10 前创建唯一 pre-v10 备份并事务迁移；回滚必须保留 v10 主库并使用对应备份或隔离目录。
...
- 当前 S3-R3 候选为 schema v10，并在 v9 → v10 前建立 `pre-v10` 备份。V0.4.0 不能直接打开 v9/v10；回滚时必须保留新主库并使用对应 pre-v9/pre-v10 备份或隔离数据目录。
