# 元枢产品事实（V0.4.0 冻结基线 / V2 阶段 3 R3 候选）

> 当前产品事实的唯一入口。更新时间：2026-08-30。V0.4.0 阶段 2 已冻结；S3-R1/R2 已集成，当前源码正在验证 S3-R3 逐 Turn 记忆出站确认候选，它不是新的正式发布版本。
...
- **阶段 3 R1 / R2 / R3**：已有本机加密记忆账本和确定性预览；R3 候选允许用户为单个 Turn 选择记忆，并在查看完整 Provider、HTTPS 去向、项目绑定和正文后单次确认发送。默认仍为 0 条，不自动提取或后台发送。
...
- SQLite/Desktop IPC 当前候选合同为 v10。直接 v8→v10 创建唯一 `pre-v10-from-v8`，v9→v10 创建唯一 `pre-v10-from-v9`；各版本步骤单独事务提交，失败保留该步骤开始前的版本。回滚必须保留新主库并使用匹配来源的备份或隔离目录。
...
- S3-R3 仍没有自动记忆提取、后台/语义检索、RAG、向量数据库、用户画像或跨 Session 自动个性化；`intent.semantic` 永不接收记忆。
...
- 当前 S3-R3 候选为 schema v10。直接从 V0.4.0 schema v8 升级只建立 `pre-v10-from-v8`，不会自动建立中间 pre-v9；从 v9 升级建立 `pre-v10-from-v9`。V0.4.0 不能直接打开 v9/v10；回滚时必须保留新主库，并使用匹配来源的 pre-v10、既有 v8 备份或隔离数据目录。
