...
- Vision diagnostic：仅在正式批次持续出现无法由普通标记比较解释的 `vision.canary_missing` 后使用；重新取得可见批次同意，只捕获 1 次 Runner 自建测试窗口。窗口同时显示 4 组固定非个人候选标记，报告只保留固定候选 ID、预期长度、紧凑文本总长度和最小编辑距离，不输出标记或 OCR 正文。诊断样本不计入正式成功率，也不能替代正式批次。
...
允许：合同版本、exact SHA、模式、粗粒度 Windows build/架构、能力布尔值、麦克风数量桶、次数、四类终态、聚合耗时、稳定错误码、清理状态、网络/Provider 请求数 0，以及仅由计数构成的语音匹配摘要（样本数、编辑距离 0/1/2/3+ 分桶、同长度不匹配、较短、较长）和视觉诊断摘要（固定候选 ID、预期长度、紧凑文本总长度、最小编辑距离）。
