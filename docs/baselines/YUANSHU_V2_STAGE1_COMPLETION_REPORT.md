...
最终源码提交、annotated tag object、标签后重建的安装包版本与 SHA-256 均已写入基线。标签之后的文档证据提交只记录发布事实，不改变标签所指源码或发布二进制来源。
...
最终 Release 综合结果：5/5，`Failed=0`、`FalseCompleted=0`。运行证据：`%LOCALAPPDATA%\ScreenGuide\Experiments\DesktopV01\20260823-184833`。
...
- 真实 Notepad 窗口同意：严格校验记事本窗口句柄与标题，只捕获本 Turn 显示并获准的单个窗口，Turn 为 `Completed`；
...
- V0.3.0 最终源码提交：`0a8cd9e164c35b86f67ffd94b9e0f17c312a2576`。
- V0.3.0 标签：`v0.3.0-stage1`。
- annotated tag object：`9fc790ade57fa2d3c18bc5ee84e8dc9e7018aa89`。
- 自动化全量结果：363/363 通过，失败 0，跳过 0。
- DesktopClient / DesktopHost ProductVersion：`0.3.0+0a8cd9e164c35b86f67ffd94b9e0f17c312a2576`；FileVersion：`0.3.0.0`。
- 发布目录文件数：533。
- 正式安装包：`artifacts/release/元枢-V0.3.0-安装包.exe`，64,039,656 bytes，`NotSigned`。
- 正式安装包 SHA-256：`42C609E130B29C6D96784C2B0266473B6D3417BE0DC5FE9C81C7517CB100FCC7`。

版本冻结采用两提交方式：源码与预冻结文档提交 `0a8cd9e164c35b86f67ffd94b9e0f17c312a2576` 是 `v0.3.0-stage1` 的目标；从该标签目标重建并计算安装包哈希后，再由标签之后的仅文档证据提交记录上述身份，避免自引用。后续文档提交不属于标签源码，也不改变发布二进制。
...
阶段 1 的产品、架构、安全、自动化、真实用户流程和源码/产物身份冻结均已通过，V0.3.0 的源码、标签与安装包可以精确追溯，因此建议进入阶段 2。安装包对外分发前仍须在干净机完成安装—卸载—重装验收；阶段 2 也仍需由项目负责人正式启动。
