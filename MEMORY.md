...
- S5-R1 packaging/NOTICE 前置基础设施状态为 `FAIL_CLOSED_INFRASTRUCTURE_IMPLEMENTED`：受版本控制的 bundle/index skeleton、离线 validator、release-script 前置调用和 Inno mandatory include 已接线；只有四轴全 `VERIFIED`、exact 文件/hash 与 payload/container 映射全部闭合时才生成精确文件清单。当前真实 skeleton 返回 `distribution_notice_bundle_incomplete`，在 restore、publish、installer compile 之前停止；未执行 installer，`NOTICE_IMPLEMENTATION_PENDING` 与 `EXTERNAL_DISTRIBUTION_BLOCKED` 不变。
